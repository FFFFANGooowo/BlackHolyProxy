// X.AI API proxy for local Deno server
...
async function handler(req: Request): Promise<Response> {
  ...
  // Handle root path
  if (url.pathname === '/') {
    return new Response('X.AI API Proxy (Local Deno) is Running', {
      status: 200,
      headers: {
...
// Start the server (本地监听 0.0.0.0:8000)
console.log("Proxy server running on http://0.0.0.0:8000");
